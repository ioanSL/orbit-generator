...
def background(x, y, t=0.4, red=0.1):

    color = [1.2, 5.1, 0.2, 0.5]
    pattern = [0.7, 0.1, 0.4, 0.9]
    form_x = [.1, .2, .3, .4]
    form_y = [.9, .8, .7, .6]

    for i in range(0, 3):
        zeros = np.zeros_like(x)
        p = np.array([x / 4 + 1, y / 4 - 1.3, -1 + zeros])
        p += 0.1 * (np.array([zeros + 1.0, zeros + 0.5, zeros + np.sin(t / 12.)
                              ]))

        t1 = field(p[0, :], p[1, :], p[2, :], t)
        v = (pattern[i] - np.exp((np.abs(x) - 1.) * 6.)) * (pattern[i] - np.exp(
            (np.abs(y) - 1.) * 6.))

        c1 = (form_x[i] + form_y[i] * v) * np.array([red * t1 * t1 * t1, color[i] * t1 * t1, t1])
    return c1
